  ...
  (let* ((ntset (remove-duplicates (mapcar #'car (grammar-rules grammar)) :test #'equal))
	...
	(setf (aref symbol-array (cdr sym-num)) (car sym-num)))
      ...
      (make-canonical-grammar
       ...
       nt-num t-num symbol-array symbol-alist))))

(defstruct (canonical-grammar
	    (:conc-name cg-)
	    (:constructor make-canonical-grammar
	      (start rules nt-num t-num sym-array sym-alist)))
  start
  rules
  nt-num
  t-num
...
(defun sequence-nullability (cg sequence)
  (let ((nullable t))
    (loop while nullable
	  for s in sequence
	...
	(destructuring-bind (symbol . num) arrow
	  (when (< symbol (cg-nt-num cg))
	    (push (cons (cons (car row) symbol)
			(mapcar #'car (remove-if #'(lambda (arrow) (< (car arrow) (cg-nt-num cg))) (cdr (assoc num goto-table)))))
		  dr)))))))

;; checker
(defun check-dr-set (cg &key (start "START") (eof "EOF"))
  (setf 
   (aref (cg-sym-array cg) 0) start
   (aref (cg-sym-array cg) (cg-t-num cg)) eof)
  (mapcar
   #'(lambda (x) (cons (cons (caar x) (aref (cg-sym-array cg) (cdar x)))
		       (mapcar #'(lambda (y) (aref (cg-sym-array cg) y)) (cdr x))))
